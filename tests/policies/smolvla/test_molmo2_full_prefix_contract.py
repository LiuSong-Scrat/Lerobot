...
def test_public_config_loader_rejects_missing_raw_topology_before_defaults(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="no valid WEP-prefix topology marker"):
...
def test_full_prefix_inserts_trainable_scene_and_keeps_variable_text_right_padded() -> None:
    ...
    # Exact WEP prefix: all valid IMAGE/LANGUAGE/FG/BG tokens are mutually
    # visible, not Molmo's native causal-text hybrid mask.
    assert not boundaries.any()
    allowed = make_att_2d_masks(prefix_valid, boundaries)
    expected = prefix_valid[:, :, None] & prefix_valid[:, None, :]
...
def test_world_fg_bg_append_to_same_prefix_and_reuse_ego_positions() -> None:
    torch.manual_seed(5)
    model = VLAFlowMatching.__new__(VLAFlowMatching)
    nn.Module.__init__(model)
    ...
    prefix_boundaries = torch.zeros_like(prefix_valid)
    ...
    ego_indices = model.last_molmo_scene_insert_positions[:, None] + torch.arange(2)[None]
    assert torch.equal(positions[:, -2:], base_positions.gather(1, ego_indices))
...
def _joint_attention_and_positions(
    prefix_valid: torch.Tensor, action_valid: torch.Tensor
...
def test_action_loss_crosses_all_36_frozen_vlm_layers_into_all_scene_prefixes() -> None:
    ...
    assert prefix.requires_grad
    assert not prefix_boundaries.any()
...
def test_full_prefix_cache_matches_joint_forward_with_action_only_suffix() -> None:
    ...
    with torch.no_grad():
        ...
        (_, _), cache = backend(
            attention_mask=prefix_valid[:, :, None] & prefix_valid[:, None, :],
