...
class SmolVLAConfig(PreTrainedConfig):
    ...
    # Direct trajectory losses in Ego/body and Overview/world coordinates.
    worldflow_loss_weight: float = 0.05
    worldflow_geo_loss_weight: float = 0.05
    worldflow_bridge_loss_weight: float = 0.05
    # The Action Expert only follows the direct SE(3) trajectory head after its
    # metric trajectory error is small. This avoids using a randomly initialized
    # auxiliary head as a teacher.
    worldflow_bridge_confidence_tau: float = 0.05
    worldflow_bridge_rotation_radius: float = 0.08
    # Metric weighting for direct SE(3) trajectory supervision.
    ...
    def __post_init__(self):
        ...
        if self.worldflow_enable:
            ...
            if self.worldflow_min_transport_points < 3:
                raise ValueError("worldflow_min_transport_points must be at least 3 for reliable co-motion evidence.")
        ...
        if self.worldflow_se3_head_enable:
            warnings.warn(
                ...
                "WorldFlow now always predicts a direct SE(3) trajectory and uses analytic World/Ego conjugation.",
                stacklevel=2,
            )
