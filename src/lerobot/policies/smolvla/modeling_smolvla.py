...
class VLAFlowMatching(nn.Module):
    ...
    def __init__(self, config: SmolVLAConfig, rtc_processor: RTCProcessor | None = None):
        ...
        scene_hidden_size = (
            ...
            if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full"
            else self.vlm_with_expert.config.text_config.hidden_size
        )
        ...
        if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full":
            # Trainable scene tokens use Molmo's native embedding scale while
            # remaining entirely on the Action Expert side of the freeze boundary.
        ...
        self.last_expert_scene_tokens: Tensor | None = None
        self.last_expert_scene_mask: Tensor | None = None
        ...
        if self.worldflow_branch is not None:
            ...
            scene_prefix_dim = (
                expert_dim if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full" else vlm_dim
            )
    ...
    def _build_full_molmo_native_prefix(
        ...
        *,
        ...
        """Return frozen native IMAGE/TEXT tokens and Molmo's hybrid block boundaries."""

        if native_embeddings.ndim != 3:
            raise ValueError(f"Expected native embeddings [B,L,H], got {native_embeddings.shape}.")
        batch_size, native_length = native_embeddings.shape[:2]
        ...
        valid = native_valid.to(device=native_embeddings.device, dtype=torch.bool)
        image_tokens = native_image_tokens.to(device=native_embeddings.device, dtype=torch.bool)
        image_counts = (image_tokens & valid).sum(dim=1)
        if not bool((image_counts == 410).all()):
            raise ValueError(f"Expected exactly 410 native IMAGE positions, got {image_counts.tolist()}.")

        text_role, image_role, pad_role = 0, 1, 3
        roles = torch.where(
            valid,
            torch.where(
                image_tokens,
                torch.full_like(image_tokens, image_role, dtype=torch.int8),
                torch.full_like(image_tokens, text_role, dtype=torch.int8),
            ),
            torch.full_like(image_tokens, pad_role, dtype=torch.int8),
        )
        if ablate_language:
            text_positions = roles == text_role
            text_positions[:, 0] = False
            valid = valid & ~text_positions

        boundaries = (roles == text_role) & valid
        for batch_index in range(batch_size):
            image_positions = torch.nonzero(image_tokens[batch_index] & valid[batch_index])
            if image_positions.numel() == 0:
                raise ValueError("Each Full Molmo sample must retain a native image block.")
            boundaries[batch_index, int(image_positions[0].item())] = True
        boundaries[~valid] = False

        self.last_molmo_scene_insert_positions = None
        self.last_molmo_token_roles = roles.detach()
        self.last_prefix_token_layout = ("bos", "native_image", "causal_text")
        image_values = native_embeddings.detach().float()[roles == image_role]
        text_values = native_embeddings.detach().float()[(roles == text_role) & valid]
        self.last_prefix_metrics.update(
            {
                "image_prefix_rms": image_values.square().mean().sqrt(),
                "language_prefix_rms": text_values.square().mean().sqrt(),
                "vlm_inference_only": torch.ones((), device=native_embeddings.device),
            }
        )
        # The backend repeats detach/no_grad checks before every decoder layer;
        # detaching here makes the VLM boundary explicit at the policy seam too.
        return native_embeddings.detach(), valid, boundaries
    ...
    def embed_prefix(
        ...
        self.last_expert_scene_tokens = None
        self.last_expert_scene_mask = None
        ...
        for _pc_idx, (
            ...
            if is_molmo_full:
                ...
                self.last_expert_scene_tokens = pc_emb
                self.last_expert_scene_mask = pc_mask.to(device=pc_emb.device, dtype=torch.bool)
            else:
        ...
        if is_molmo_full:
            if self.last_expert_scene_tokens is None or self.last_expert_scene_mask is None:
                raise RuntimeError(
    ...
    def _append_shared_expert_world_scene_prefix(
        ...
        if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full":
            # Frozen Molmo consumes native IMAGE/TEXT only. The exact v0.4.3
            # World scene pair is moved to the trainable Expert suffix below.
            base_position_ids = torch.cumsum(
    ...
    def _build_shared_point_action_expert_joint_suffix(
        ...
        action_positions = (
            ...
            + (2 if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full" else 0)
        )
        pieces: list[tuple[str, Tensor, Tensor, Tensor]] = []
        if getattr(getattr(self, "config", None), "vlm_backend", "smolvlm") == "molmo2_full":
            required_scene_modules = (
                self.world_pointseg_object_proj,
                self.world_pointseg_background_proj,
                self.world_point_prefix_type_embedding,
            )
            if any(module is None for module in required_scene_modules):
                raise RuntimeError(
                    "Full Molmo DoubleFlow World foreground/background projections are not initialized."
                )
            ego_scene = self.last_expert_scene_tokens
            ego_scene_mask = self.last_expert_scene_mask
            world_scene_features = world_tokens.get("scene_global_tokens")
            world_scene_mask = world_tokens.get("scene_global_mask")
            if (
                not torch.is_tensor(ego_scene)
                or not torch.is_tensor(ego_scene_mask)
                or not torch.is_tensor(world_scene_features)
                or not torch.is_tensor(world_scene_mask)
            ):
                raise ValueError(
                    "Full Molmo DoubleFlow requires Ego and World foreground/background scene pairs."
                )
            if ego_scene.shape[1] != 2 or world_scene_features.shape[1] != 2:
                raise ValueError(
                    "Full Molmo DoubleFlow scene layout must be "
                    "[Ego FG, Ego BG, World FG, World BG]."
                )
            world_scene = torch.stack(
                [
                    self.world_pointseg_object_proj(world_scene_features[:, 0]),
                    self.world_pointseg_background_proj(world_scene_features[:, 1]),
                ],
                dim=1,
            )
            world_scene = world_scene + self.world_point_prefix_type_embedding
            ego_scene_mask = ego_scene_mask.to(device=ego_scene.device, dtype=torch.bool)
            world_scene_mask = world_scene_mask.to(device=world_scene.device, dtype=torch.bool)
            if "world" in getattr(self, "inference_ablation_modalities", frozenset()):
                world_scene_mask = torch.zeros_like(world_scene_mask)
            batch_size = ego_scene.shape[0]
            role_zero = torch.zeros(batch_size, 1, dtype=torch.long, device=ego_scene.device)
            role_one = torch.ones(batch_size, 1, dtype=torch.long, device=ego_scene.device)
            pieces.extend(
                [
                    ("ego_foreground", ego_scene[:, 0:1], ego_scene_mask[:, 0:1], role_zero),
                    ("ego_background", ego_scene[:, 1:2], ego_scene_mask[:, 1:2], role_one),
                    (
                        "world_foreground",
                        world_scene[:, 0:1],
                        world_scene_mask[:, 0:1],
                        role_zero.to(device=world_scene.device),
                    ),
                    (
                        "world_background",
                        world_scene[:, 1:2],
                        world_scene_mask[:, 1:2],
                        role_one.to(device=world_scene.device),
                    ),
                ]
            )
        pieces.extend(
            [
                ("ego_action", ego_action, ego_action_mask, action_positions),
                (
                    "world_action",
                    world_action,
                    world_action_mask,
                    action_positions.to(device=world_action.device),
                ),
            ]
        )
