...
class TaskBalancedFrameSampler:
    """Draw an equal number of eligible frames from every episode-level task."""
    ...
    def __init__(
        ...
        if not (len(dataset_from_indices) == len(dataset_to_indices) == len(episode_group_ids)):
            raise ValueError("Episode boundaries and episode_group_ids must have identical lengths.")
        selected = None if episode_indices_to_use is None else set(episode_indices_to_use)
    ...
    def __iter__(self) -> Iterator[int]:
        per_group, remainder = divmod(self.num_samples, len(self.group_ids))
