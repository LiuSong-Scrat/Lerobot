...
typedef int simInt;
// CoppeliaSim 4.10 uses double precision for the legacy API by default.
...
typedef int (*contactCallback)(int,int,int,int*,simFloat*);
typedef int (*jointCtrlCallback)(int,int,int,const int*,const simFloat*,simFloat*);
...
simInt simAddDrawingObject(simInt objectType,simFloat size,simFloat duplicateTolerance,simInt parentObjectHandle,simInt maxItemCount,const float* ambient_diffuse,const float* setToNULL,const float* specular,const float* emission);
...
simInt simSetLightParameters(simInt objectHandle,simInt state,const float* setToNULL,const float* diffusePart,const float* specularPart);
simInt simGetLightParameters(simInt objectHandle,float* setToNULL,float* diffusePart,float* specularPart);
...
simInt simSetShapeColor(simInt shapeHandle,const simChar* colorName,simInt colorComponent,const float* rgbData);
simInt simGetShapeColor(simInt shapeHandle,const simChar* colorName,simInt colorComponent,float* rgbData);
...
unsigned char* simGetVisionSensorImg(simInt sensorHandle, simInt options, simFloat rgbaCutOff, const simInt* pos, const simInt* size, simInt* resolution);
float* simGetVisionSensorDepth(simInt sensorHandle, simInt options, const simInt* pos, const simInt* size, simInt* resolution);
...
float* simGetVisionSensorImage(simInt visionSensorHandle);
simUChar* simGetVisionSensorCharImage(simInt visionSensorHandle,simInt* resolutionX,simInt* resolutionY);
float* simGetVisionSensorDepthBuffer(simInt visionSensorHandle);
simInt simSetVisionSensorImage(simInt visionSensorHandle,const float* image);
...
simInt simCreateDummy(simFloat size,const float* color);
...
simInt simCreatePath(simInt attributes,const simInt* intParams,const simFloat* floatParams,const float* color);
...
const simFloat* simGetOctreeVoxels(simInt octreeHandle,simInt* ptCnt,simVoid* reserved);
const simFloat* simGetPointCloudPoints(simInt pointCloudHandle,simInt* ptCnt,simVoid* reserved);
...
ffibuilder.set_source(
    ...
    """
         #include "sim_compat.h"   // CoppeliaSim 4.10 ABI compatibility
    """,
