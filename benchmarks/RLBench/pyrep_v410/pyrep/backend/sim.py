...
def simGetScriptHandleEx(scriptType, objectHandle=-1, scriptName=''):
    """Resolve a script using CoppeliaSim 4.10's extended API."""
    if scriptName is None:
        scriptName = ''
    handle = lib.simGetScriptHandleEx(
        int(scriptType), int(objectHandle), scriptName.encode('ascii'))
    return int(handle)
...
def simGetJointForce(jointHandle):
    force = ffi.new('double *')
    ret = lib.simGetJointForce(jointHandle, force)
    _check_return(ret)
    if ret == 0:
        # CoppeliaSim 4.10 may not expose a dynamics force on the first
        # observation immediately after a scene reset. RLBench treats this
        # field as an observation, so keep the reset observation valid and
        # report the neutral value until the first dynamics sample exists.
        return 0.0
    return force[0]
...
def simReadForceSensor(forceSensorHandle):
    forceVector  = ffi.new('double[3]')
    torqueVector = ffi.new('double[3]')
...
def simGetVisionSensorImage(sensorHandle, resolution):
    if os.environ.get('PYREP_V410_USE_MODERN_VISION_API', '1') == '1':
        resolution_out = ffi.new('int[2]')
        pos = ffi.new('int[2]', [0, 0])
        size = ffi.new('int[2]', [0, 0])
        img_buffer = lib.simGetVisionSensorImg(
            sensorHandle, 0, 0.0, pos, size, resolution_out)
        width, height = int(resolution_out[0]), int(resolution_out[1])
        if width <= 0 or height <= 0:
            raise RuntimeError('CoppeliaSim returned an invalid vision resolution')
        img = np.frombuffer(
            ffi.buffer(img_buffer, width * height * 3), np.uint8)
...
def simGetVisionSensorDepthBuffer(sensorHandle, resolution, in_meters):
    if os.environ.get('PYREP_V410_USE_MODERN_VISION_API', '1') == '1':
        ...
        img_buffer = lib.simGetVisionSensorDepth(
            sensorHandle, options, pos, size, resolution_out)
        width, height = int(resolution_out[0]), int(resolution_out[1])
        if width <= 0 or height <= 0:
            raise RuntimeError('CoppeliaSim returned an invalid depth resolution')
        img = np.frombuffer(
            ffi.buffer(img_buffer, width * height * 4), np.float32)
...
def simReadProximitySensor(sensorHandle):
    detectedPoint = ffi.new('double[3]')
    detectedObjectHandle = ffi.new('int *')
    detectedSurfaceNormalVector = ffi.new('double[3]')
...
def simReadDistance(distanceObjectHandle):
    minimumDistance = ffi.new('double *')
...
def simGetObjectVelocity(objectHandle):
    linearVel = ffi.new('double[3]')
    angularVel = ffi.new('double[3]')
...
def simExtCallScriptFunction(functionNameAtScriptName, scriptHandleOrType,
    ...
    # CoppeliaSim 4.10 no longer resolves an add-on by the legacy
    # ``function@PyRep`` name when the caller passes the add-on script type.
    # Resolve the named add-on once and call the function by its local name.
    if (scriptHandleOrType == sim_scripttype_addonscript
            and functionNameAtScriptName.endswith('@PyRep')):
        scriptHandleOrType = simGetScriptHandleEx(
            sim_scripttype_addonscript, -1, 'PyRep')
        if scriptHandleOrType < 0:
            raise RuntimeError('CoppeliaSim 4.10 PyRep add-on is not loaded')
        functionNameAtScriptName = functionNameAtScriptName[:-len('@PyRep')]
...
def _v410_ik_inputs(base_handle, tip_handle, target_handle, collision_handle,
                    ignore_collisions, joint_handles, locked_handles=(),
                    mode='sampling', distance_threshold=0.65,
                    max_time_ms=10, trials=1, max_configs=1,
                    path_point_count=None):
    """Build arguments for the isolated CoppeliaSim 4.10 simIK bridge."""
    joint_handles = list(joint_handles)
    locked_handles = list(locked_handles)
    if path_point_count is None:
        ints = [base_handle, tip_handle, target_handle, collision_handle,
                int(ignore_collisions), int(trials), int(max_configs),
                len(joint_handles), len(locked_handles)]
    else:
        ints = [base_handle, tip_handle, target_handle, collision_handle,
                int(ignore_collisions), int(path_point_count), 0,
                len(joint_handles), len(locked_handles)]
    ints.extend(joint_handles)
    ints.extend(locked_handles)
    return ints, [float(distance_threshold), float(max_time_ms) / 1000.0], [mode]


def v410SolveIK(base_handle, tip_handle, target_handle, collision_handle,
                ignore_collisions, joint_handles, position_distance=0.65,
                max_time_ms=10, trials=1, max_configs=1,
                mode='sampling', locked_handles=()):
    """Solve an IK target through CoppeliaSim 4.10's simIK plugin."""
    ints, floats, strings = _v410_ik_inputs(
        base_handle, tip_handle, target_handle, collision_handle,
        ignore_collisions, joint_handles, locked_handles, mode,
        position_distance, max_time_ms, trials, max_configs)
    script_handle = simGetScriptHandleEx(
        sim_scripttype_addonscript, -1, 'PyRep')
    if script_handle < 0:
        raise RuntimeError('CoppeliaSim 4.10 PyRep add-on is not loaded')
    ret_ints, ret_floats, _, _ = simExtCallScriptFunction(
        'v410SolveIK', script_handle,
        ints, floats, strings, '')
    status = ret_ints[0] if ret_ints else 0
    detail = ret_ints[1] if len(ret_ints) > 1 else 0
    return status, detail, ret_floats


def v410GenerateIKPath(base_handle, tip_handle, target_handle,
                       collision_handle, ignore_collisions, joint_handles,
                       point_count, max_time_ms=10):
    """Generate a straight configuration path with the 4.10 simIK plugin."""
    ints, floats, strings = _v410_ik_inputs(
        base_handle, tip_handle, target_handle, collision_handle,
        ignore_collisions, joint_handles, mode='path',
        max_time_ms=max_time_ms, path_point_count=point_count)
    script_handle = simGetScriptHandleEx(
        sim_scripttype_addonscript, -1, 'PyRep')
    if script_handle < 0:
        raise RuntimeError('CoppeliaSim 4.10 PyRep add-on is not loaded')
    _, ret_floats, _, _ = simExtCallScriptFunction(
        'v410GenerateIKPath', script_handle,
        ints, floats, strings, '')
    return ret_floats
...
def simCheckIkGroup(ikGroupHandle, jointHandles):
    jointValues = ffi.new('double[%d]' % len(jointHandles))
...
def simInvertMatrix(matrix):
    c_matrix = ffi.new('double []', matrix)
...
def simMultiplyMatrices(inMatrix1, inMatrix2):
    outMatrix = ffi.new('double []', len(inMatrix1))
...
def simGetContactInfo(contact_obj_handle, get_contact_normal):
    ...
    while result > 0:
        if get_contact_normal:
            contact = ffi.new('double[9]')
            ext = sim_handleflag_extended
        else:
            contact = ffi.new('double[6]')
            ext = 0
...
def simGetConfigForTipPose(ikGroupHandle, jointHandles, thresholdDist, maxTimeInMs, metric, collisionPairs, jointOptions, lowLimits, ranges):
    ...
    retConfigm = ffi.new('double[%d]' % jointCnt)
    reserved = ffi.NULL
