...
class Arm(RobotComponent):
    ...
    def solve_ik_via_sampling(self,
        ...
        status, _, ret_floats = sim.v410SolveIK(
            self.get_handle(), self._ik_tip.get_handle(),
            self._ik_target.get_handle(), self._collision_collection,
            ignore_collisions, handles,
            position_distance=distance_threshold,
            max_time_ms=max_time_ms,
            trials=trials,
            max_configs=max_configs,
            mode='sampling')
        valid_joint_positions = []
        if status == 1 and len(ret_floats) >= len(handles):
            config_count = len(ret_floats) // len(handles)
            valid_joint_positions = [
                ret_floats[i * len(handles):(i + 1) * len(handles)]
    ...
    def solve_ik_via_jacobian(
        ...
        handles = [j.get_handle() for j in self.joints]
        locked_handles = [] if locked_joints is None else [
            handles[idx] for idx in locked_joints]
        ik_result, _, joint_values = sim.v410SolveIK(
            self.get_handle(), self._ik_tip.get_handle(),
            self._ik_target.get_handle(), self._collision_collection,
            True, handles, mode='jacobian', locked_handles=locked_handles)
        if ik_result == 0:
    ...
    def get_linear_path(self, position: Union[List[float], np.ndarray],
        ...
        if euler is not None:
            ...
            self._ik_target.set_quaternion(quaternion, relative_to)
        handles = [j.get_handle() for j in self.joints]
        ret_floats = sim.v410GenerateIKPath(
            self.get_handle(), self._ik_tip.get_handle(),
            self._ik_target.get_handle(), self._collision_collection,
            ignore_collisions, handles, max(2, int(steps)), max_time_ms=10)
        self._ik_target.set_pose(prev_pose)
        if len(ret_floats) == 0:
    ...
    def get_nonlinear_path(self, position: Union[List[float], np.ndarray],
        ...
        # The old PyRep addon eventually calls simGenerateIkPath, which is a
        # no-op in CoppeliaSim 4.10.  simIK.generatePath performs the same
        # Cartesian interpolation while preserving the target pose semantics.
        prev_pose = self._ik_target.get_pose()
        self._ik_target.set_position(position, relative_to)
        if euler is not None:
            self._ik_target.set_orientation(euler, relative_to)
        ...
        ret_floats = sim.v410GenerateIKPath(
            self.get_handle(), self._ik_tip.get_handle(),
            self._ik_target.get_handle(), self._collision_collection,
            ignore_collisions, handles, max(2, int(trials)),
            max_time_ms=max_time_ms)
        self._ik_target.set_pose(prev_pose)
