...
def visualize_res(batch, result, batch_idx=0, ood_test_sno=0, step=0, output_dir: str | Path | None = None):
    ...
    # ================= ת\BB\BB\B2\A2\BAϲ\A2 =================
    all_points = []
    all_colors = []

    # 1. \CC\ED\BCӳ\A1\BE\B0\B5\E3\D4Ƶĵ\E3\BA\CD\D1\D5ɫ
    ...
    # 2. \BD\ABÿ\B8\F6\D7\F8\B1\EA\D6\E1\CD\F8\B8\F1\B2\C9\D1\F9Ϊ\B5\E3\D4\C6
    for frame in geometries:
        if isinstance(frame, o3d.geometry.TriangleMesh):
            # \B4\D3\CD\F8\B8\F1\D6в\C9\D1\F9\B5\E3
            frame_pcd = frame.sample_points_poisson_disk(number_of_points=100) # ÿ\B8\F6\D7\F8\B1\EA\D6\E1\B2\C9\D1\F9100\B8\F6\B5\E3
            all_points.append(np.asarray(frame_pcd.points))
            all_colors.append(np.asarray(frame_pcd.colors))

    # 3. \BAϲ\A2\CB\F9\D3е\E3\BA\CD\D1\D5ɫ
    final_points = np.vstack(all_points)
    final_colors = np.vstack(all_colors)

    # 4. \B4\B4\BD\A8\D7\EE\D6յĵ\E3\D4ƶ\D4\CF\F3
    ...
    if o3d.io.write_point_cloud(str(ply_save_path), final_pcd):
        print(f"\BAϲ\A2\BA\F3\B5ĵ\E3\D4\C6\D2ѱ\A3\B4\E6Ϊ {ply_save_path}")
    else:
        logging.warning(f"\BAϲ\A2\BA\F3\B5ĵ\E3\D4Ʊ\A3\B4\E6ʧ\B0\DC: {ply_save_path}")
...
def update_policy(
    ...
    with accelerator.autocast():
        ...
        if rabc_batch_weights is not None:
            ...
            # Apply RA-BC weights: L_RA-BC = \A6\B2(w_i * l_i) / (\A6\B2w_i + \A6\C5)
    ...
    for _ in range(step, cfg.steps):
        ...
        if cfg.env and is_eval_step:
            if is_main_process:
                ...
                # meters/tracker
                eval_metrics = {
                    "avg_sum_reward": AverageMeter("\A1\C6rwrd", ":.3f"),
