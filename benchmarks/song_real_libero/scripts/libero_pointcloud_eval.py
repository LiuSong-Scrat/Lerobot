...
def correct_rim_grasp_target_world(
    ...
    *,
    ...
    lift = float(lift)
    if abs(axis_shift) <= 1e-6 and abs(lift) <= 1e-6:
        return target_world, metrics

    corrected = target_world.copy()
    local_shift = np.asarray([axis_shift, 0.0, 0.0], dtype=np.float32)
    corrected[:3, 3] = corrected[:3, 3] + corrected[:3, :3] @ local_shift
    corrected[2, 3] = corrected[2, 3] + lift
    metrics.update({"applied": True, "axis_shift": axis_shift, "lift": lift})
...
def run_episode(
    *,
    ...
    gripper_close_threshold = float(control.get("gripper_close_threshold", 0.07))
    ...
    for step in range(max_steps):
        ...
        if (replan_every_step and not gripper_waiting and not pose_waiting) or not action_queue:
        ...
        rim_correction_metrics = {"applied": False, "axis_shift": 0.0, "lift": 0.0, "point_count": 0}
        ...
        if gripper_should_close and not pre_pose_done:
            gripper_command = 0.0
            active_gripper_control = False
        if pre_pose_done and not pre_gripper_done:
            motion_action = hold_motion_action.copy()
        if pre_pose_done and pre_gripper_done:
            motion_action = hold_motion_action.copy()
            gripper_command = 0.0
            active_gripper_control = False
        ...
        should_wait_gripper = (
            ...
            and (pose_done or not gripper_should_close)
        )
        if should_wait_pose:
            pose_waiting = True
            pose_wait_steps += 1
        ...
        if should_wait_gripper or should_wait_pose:
            pass
        else:
            action_queue.pop(0)
                ...
                try:
                    for episode_idx in range(cfg["episodes"]):
                        ...
                        try:
                            ...
                            # np.save(episode_dir / "controller_pose_actions.npy", result["controller_pose_actions"])
                            # np.save(episode_dir / "model_pose_actions.npy", result["model_pose_actions"])
                            # np.save(episode_dir / "pose_pos_errors.npy", result["pose_pos_errors"])
                            # np.save(episode_dir / "pose_rot_errors.npy", result["pose_rot_errors"])
                            # np.save(episode_dir / "pose_wait_flags.npy", result["pose_wait_flags"])
                            # np.save(episode_dir / "gripper_targets.npy", result["gripper_targets"])
                            # np.save(episode_dir / "gripper_actuals.npy", result["gripper_actuals"])
                            # np.save(episode_dir / "gripper_width_errors.npy", result["gripper_width_errors"])
                            # np.save(episode_dir / "gripper_wait_flags.npy", result["gripper_wait_flags"])
                            # np.save(episode_dir / "gripper_contact_stall_flags.npy", result["gripper_contact_stall_flags"])
                            # np.save(episode_dir / "gripper_rim_correction_flags.npy", result["gripper_rim_correction_flags"])
                            # np.save(episode_dir / "gripper_rim_axis_shifts.npy", result["gripper_rim_axis_shifts"])
                            # np.save(episode_dir / "gripper_rim_lifts.npy", result["gripper_rim_lifts"])
                            # np.save(episode_dir / "gripper_rim_point_counts.npy", result["gripper_rim_point_counts"])
                            # np.save(episode_dir / "gripper_progress_indices.npy", result["gripper_progress_indices"])
