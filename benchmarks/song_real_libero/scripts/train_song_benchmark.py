...
def audit_full_molmo2er_policy(policy: torch.nn.Module) -> dict[str, int]:
    ...
    if getattr(config, "molmo_inference_only", None) is not True:
        raise RuntimeError("Full-Molmo2-ER Scheme B requires molmo_inference_only=true.")
    ...
    drift = {
        ...
        for key, expected in scheme_b_contract.items()
        if architecture.get(key) != expected
    }
    if drift or getattr(backend, "inference_only_vlm", None) is not True:
        raise RuntimeError(f"Full-Molmo2-ER Scheme-B execution contract drifted: {drift}.")
