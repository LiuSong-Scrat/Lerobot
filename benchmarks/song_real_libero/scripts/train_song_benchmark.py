...
def train(cfg: TrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    expected_image_keys = {f"observation.images.{view}" for view in selected_views}
    ...
    if missing_image_keys and bool(getattr(cfg.policy, "vla_adapter_enable", False)):
        raise ValueError(
            f"Selected camera views {selected_views} require image features {missing_image_keys}, "
            f"but policy image features are {sorted(actual_image_keys)}."
        )
    if is_main_process:
        logging.info("Training camera_views=%s; image_features=%s", selected_views, sorted(actual_image_keys))
