...
aggregate=${V51R1_STEP_SCREEN_ARTIFACT:-"$joint_artifact_root/v51r1childenvfix_step${step6}_stratified_3arm_multiview_worldflow_screen.json"}
...
if [[ ! -s "$dual_artifact" ]]; then
  run_arm_background checkpoint 0 stratified_step5_dual_world "v51r1childenvfix_step${step6}_dual_world_stratified_step5"
  rejected=0
...
if (( dual_successes > 95 )); then
  if [[ ! -s "$primary_artifact" ]]; then
    run_arm primary_only 0 stratified_step5_primary_world "v51r1childenvfix_step${step6}_primary_world_stratified_step5"
  fi
...
if [[ "$primary_successes" != null ]] && (( dual_successes > primary_successes )); then
  if [[ ! -s "$causal_artifact" ]]; then
    run_arm checkpoint 1 stratified_step5_dual_worldablated "v51r1childenvfix_step${step6}_dual_worldablated_stratified_step5"
  fi
