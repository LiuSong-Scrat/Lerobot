...
runner="$root/scripts/eval_libero10_v12_one_checkpoint_4gpu_alltasks10ep_b30.sh"
...
aggregate=${V51R1_STEP_SCREEN_ARTIFACT:-"$joint_artifact_root/v51r1_step${step6}_stratified_3arm_multiview_worldflow_screen.json"}
...
label=v51r1_v32_multiscale1cm4cm_paired_symmetricpoint5e8_policy5e9_schemafix_b44
...
monitor_session=wep_v043_v51r1_convergence_monitor
...
while [[ ! -s "$checkpoint/model.safetensors" ]]; do
  if ! tmux has-session -t "=$train_session" 2>/dev/null \
    ...
    echo "V51R1 pipeline exited without final checkpoint: $checkpoint" >&2
    exit 4
  fi
  sleep 30
done
...
while [[ ! -s "$convergence_artifact" || ! -s "$drift_artifact" ]]; do
  if ! tmux has-session -t "=$monitor_session" 2>/dev/null; then
    echo "V51R1 convergence/parameter monitor exited without required final artifacts" >&2
    exit 5
  fi
  sleep 10
done
...
print("[V51R1 final training evidence] convergence and parameter-drift audits PASS")
PY
...
print(f"[V51R1 stratified preflight] PASS {p}")
PY
...
if [[ ! -s "$dual_artifact" ]]; then
  run_arm_background checkpoint 0 stratified_step5_dual_world "v51r1_step${step6}_dual_world_stratified_step5"
  rejected=0
...
if (( dual_successes > 95 )); then
  if [[ ! -s "$primary_artifact" ]]; then
    run_arm primary_only 0 stratified_step5_primary_world "v51r1_step${step6}_primary_world_stratified_step5"
  fi
...
if [[ "$primary_successes" != null ]] && (( dual_successes > primary_successes )); then
  if [[ ! -s "$causal_artifact" ]]; then
    run_arm checkpoint 1 stratified_step5_dual_worldablated "v51r1_step${step6}_dual_worldablated_stratified_step5"
  fi
...
payload={
  ...
  "next_action":"run Full500 2x2 causal gate" if passed else "do not start Full500 for V51R1",
}
