#!/usr/bin/env python3
"""Load the real V32 source on CPU and verify V49 optimizer membership."""
...
def main() -> None:
    ...
    trainable = {id(parameter): name for name, parameter in policy.named_parameters() if parameter.requires_grad}
    ...
    if group_names != EXPECTED_GROUP_NAMES or group_lrs != EXPECTED_LRS:
        raise RuntimeError(f"Unexpected V49 optimizer groups/LRs: {group_names}, {group_lrs}")
    ...
    if not exact_membership:
        raise RuntimeError("V49 optimizer grouping overlaps or omits real checkpoint parameters")
    world_only_point_role_ids = policy.get_worldflow_ego_tangent_world_only_parameter_ids()
    if not world_only_point_role_ids:
        raise RuntimeError("V49 resolved no World-only point-path gradient roles")
    if not world_only_point_role_ids < point_group_ids:
        raise RuntimeError(
            "V49 World-only point roles must be a strict subset of the high-LR point-input group"
        )
    ...
    if protected_ids & world_only_point_role_ids:
        raise RuntimeError("World-only point paths remain in the Ego-tangent protected set")
    ...
    if not ego_point_role_ids or not ego_point_role_ids <= protected_ids:
        raise RuntimeError("Ego point paths must remain in the Ego-tangent protected set")
    payload = {
        "schema_version": 2,
        "source": str(source),
        ...
        "passes_v49_real_checkpoint_optimizer_preflight": True,
    }
