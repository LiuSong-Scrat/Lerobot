...
screen=${V51R1_SCREEN_ARTIFACT:-"$joint_artifact_root/v51r1eglglobal_all_checkpoints_stratified_3arm_multiview_worldflow_screen.json"}
gate=${V51R1_STEP_GATE_ARTIFACT:-"$joint_artifact_root/v51r1eglglobal_step${step6}_full500_2x2_multiview_worldflow_causal_gate.json"}
...
if [[ ! -s "$dual_artifact" ]]; then
  run_arm_background checkpoint 0 full500_dual_world "v51r1eglglobal_step${step6}_full500_dual_world"
  rejected=0
...
if [[ ! -s "$primary_artifact" ]]; then
  run_arm_background primary_only 0 full500_primary_world "v51r1eglglobal_step${step6}_full500_primary_world"
  rejected=0
  while kill -0 "$arm_pid" 2>/dev/null; do
    sleep 10
...
if [[ ! -s "$dual_ablated_artifact" ]]; then
  run_arm_background checkpoint 1 full500_dual_worldablated "v51r1eglglobal_step${step6}_full500_dual_worldablated"
  rejected=0
  while kill -0 "$arm_pid" 2>/dev/null; do
    sleep 10
...
if [[ ! -s "$primary_ablated_artifact" ]]; then
  run_arm_background primary_only 1 full500_primary_worldablated "v51r1eglglobal_step${step6}_full500_primary_worldablated"
  wait "$arm_pid"
fi
