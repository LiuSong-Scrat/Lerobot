...
screen_session=wep_v043_v52childenvfix_all_checkpoints_stratified_3arm
screen=${V52_SCREEN_ARTIFACT:-"$joint_artifact_root/v52childenvfix_all_checkpoints_stratified_3arm_multiview_worldflow_screen.json"}
gate=${V52_STEP_GATE_ARTIFACT:-"$joint_artifact_root/v52childenvfix_step${step6}_full500_2x2_multiview_worldflow_causal_gate.json"}
...
while [[ ! -s "$screen" ]]; do
  if ! tmux has-session -t "=$screen_session" 2>/dev/null; then
    echo "V52 all-checkpoint screen exited without aggregate: $screen" >&2
    exit 4
  fi
  sleep 20
done
...
p={"status":"not_started","step":step,"reason":"V52 stratified three-arm screen did not pass for this checkpoint","screen_artifact":str(screen),"full500_arms_started":[]}
...
print("[V52 Full500 2x2 preflight] PASS")
PY
...
if [[ ! -s "$dual_artifact" ]]; then
  run_arm_background checkpoint 0 full500_dual_world "v52childenvfix_step${step6}_full500_dual_world"
  rejected=0
...
if [[ ! -s "$primary_artifact" ]]; then
  run_arm_background primary_only 0 full500_primary_world "v52childenvfix_step${step6}_full500_primary_world"
  rejected=0
  while kill -0 "$arm_pid" 2>/dev/null; do
    sleep 10
...
if [[ ! -s "$dual_ablated_artifact" ]]; then
  run_arm_background checkpoint 1 full500_dual_worldablated "v52childenvfix_step${step6}_full500_dual_worldablated"
  rejected=0
  while kill -0 "$arm_pid" 2>/dev/null; do
    sleep 10
...
if [[ ! -s "$primary_ablated_artifact" ]]; then
  run_arm_background primary_only 1 full500_primary_worldablated "v52childenvfix_step${step6}_full500_primary_worldablated"
  wait "$arm_pid"
fi
