...
dual_cache=$run_root/pointseg_cache_consensus_multiscale_novelty_union_1cm4cm
...
cache_audit=$run_root/artifacts/v52_cache_online_exact_index_audit_36shards.json
optimizer_preflight=$run_root/artifacts/v52_real_checkpoint_optimizer_and_gradient_role_preflight.json
workflow_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
optimizer_preflight_script=$workflow_root/scripts/audit_v52_real_checkpoint_optimizer_preflight.py
output=$run_root/training/v52_from_v32step100_consensus_multiscale1cm4cm_paired_symmetricpoint5e8_policy5e9_4gpu_b44_w12_1564steps
...
from lerobot.policies.smolvla import configuration_smolvla as config_module
...
audit = json.load(open(sys.argv[6], encoding="utf-8"))
expected_src = Path(sys.argv[7]).resolve() / "src"
assert Path(config_module.__file__).resolve().is_relative_to(expected_src)
assert callable(consensus_multiscale_novelty_union_sample_fused_point_cloud)
fields = config_module.SmolVLAConfig.__dataclass_fields__
...
assert dual["camera_view_fusion"] == "consensus_multiscale_novelty_union"
...
assert audit["manifest_num_samples"] == 137_590
assert audit["manifest_shard_count"] == audit["audited_samples"] == 36
paired = 2 * info["total_frames"]
global_batch = 4 * 44
assert 1563 * global_batch < paired <= 1564 * global_batch
print("V52 preflight PASS: paired epoch, consensus fine1cm/coarse4cm exact-index input")
PY
...
if [[ -z ${WANDB_MODE:-} ]]; then
  if timeout 20 "$python" -m wandb login --verify >>"$run_root/logs/wandb_v52_connectivity_preflight.log" 2>&1; then
...
exec "$python" -m accelerate.commands.launch \
  ...
  --policy.camera_view_fusion=consensus_multiscale_novelty_union \
  ...
  --job_name=wep_vla_v043_v52_v32step100_consensus1cm4cm_paired_symmetricpoint_b44_1564 \
  ...
  --wandb.run_id=joint-v52-v32step100-consensus1cm4cm-paired-symmetricpoint-b44-20260815 \
