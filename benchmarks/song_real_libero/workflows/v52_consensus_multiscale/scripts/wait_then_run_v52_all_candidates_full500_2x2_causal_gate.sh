...
worker="$workflow_root/scripts/wait_then_run_v52_checkpoint_full500_2x2_causal_gate.sh"
artifact_root="$root/joint_multiview_worldflow/libero10_500ep/artifacts"
screen="$artifact_root/v52childenvfix_all_checkpoints_stratified_3arm_multiview_worldflow_screen.json"
screen_session=wep_v043_v52childenvfix_all_checkpoints_stratified_3arm
aggregate="$artifact_root/v52childenvfix_all_candidates_full500_2x2_multiview_worldflow_causal_gate.json"
...
for step in "${candidate_steps[@]}"; do
  ...
  step_gate="$artifact_root/v52childenvfix_step${step6}_full500_2x2_multiview_worldflow_causal_gate.json"
  step_gates+=("$step_gate")
  env LEROBOT_REPO="$repo" EXPERIMENT_ROOT="$root" \
    V52_STEP="$step" V52_SCREEN_ARTIFACT="$screen" V52_STEP_GATE_ARTIFACT="$step_gate" \
    bash "$worker"
