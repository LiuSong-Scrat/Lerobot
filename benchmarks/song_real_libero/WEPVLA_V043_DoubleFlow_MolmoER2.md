...
以下均为可直接粘贴执行的命令行，不调用任何 `.sh`。完整训练使用 0–7 共 8 张 A800、普通 DDP、每卡 batch 40、global batch 320。执行前请先确认输出目录不存在或为空；不要覆盖已有训练目录。
...
下面是单卡、单任务、1 episode 的端到端推理 smoke test，用来确认 checkpoint 能加载并完成环境闭环；它不是可汇报的正式 LIBERO 分数。默认测试上面 30000-step 训练目录中的 checkpoint，若实际 checkpoint 步数不同，只替换 `--policy.path`。
...
/home/liusong/anaconda3/envs/reap/bin/python \
  ...
  --policy.path /opt/data/private/liusong/benchmarks/song_real_libero/outputs/lerobot_7B_molmo2_song/full_molmo2er_worldflow_three_stage/stage1_fresh_000000_to030000_v043data/checkpoints/030000/pretrained_model \
  ...
  --no-save-video \
  --output-dir /tmp/molmo2er_v043_libero_spatial_task0_ep1_rgbfix
```
...
cd /home/liusong/ProgramFiles/Huggingface/lerobot_7B_molmo2_song
  ...
  /home/liusong/anaconda3/envs/reap/bin/python \
    ...
    --output-dir benchmarks/song_real_libero/outputs/libero_setting/eval_molmo2er_030000
```
