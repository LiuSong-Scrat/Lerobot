...
- v25 是当前 active 实验，从 exact-baseline-compatible、exact-zero-residual 的 v14 重新训练。它针对 v20 的 Ego 回退和 v21 的 World 样本不足：correction keep 恢复到约 50%，同时 Ego/shared LR multiplier 从 `0.05` 降为 `0.005`，但严格保持非零并继续共同训练；World/bidirectional 与 residual multipliers 仍为 `0.2/4.0`。架构、单视角 10k 输入、原 action loss、BN freeze、residual-anchor gradient routing 均不变；无门控、无 World 辅助损失、无 task-specific 规则、无冻结 Ego/World。4 GPU、batch48/GPU、worker12、720 steps、W&B enabled；训练 tmux 为 `wep_v043_libero10_v25_p50_anchor_common0005_720`，Broad/Formal waiters 为 `wep_v043_v25_stratified_worldflow_causal_after_train` 和 `wep_v043_v25_full500_matched_causal_after_screen`。
