...
- 用户于V48实际step1108明确要求停止训练、直接测试。V48训练及原final-checkpoint monitor/waiter均已停止，四卡已确认释放；未保存的1108更新不进入判断，完整checkpoint只有`260/520/780/1040`。V49条件启动tmux也已取消，V49不会自动训练。新的existing-only筛查tmux为`wep_v043_v48_all_checkpoints_stratified_3arm`，日志为`joint_multiview_worldflow/libero10_500ep/logs/eval_v48_existing_steps260_520_780_1040_stratified_3arm.log`；它直接按上述四点运行fixed-barrier30/inference batch30分层100状态，正常臂第5个失败数学早停，只有`dual>95`才继续primary和World causal臂。Full500 waiter已重新启动但只对通过三臂的现有checkpoint执行，不会触发任何新训练。
