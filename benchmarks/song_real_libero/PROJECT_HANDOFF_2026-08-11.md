...
- V48 step780仍稳定、无OOM/Traceback；三个260步窗口的action/World均值依次为`2.3733e-4/2.4038e-4`、`2.3576e-4/2.3788e-4`、`2.2793e-4/2.2952e-4`，长780步前后半仅下降约`1.02%/0.64%`，状态仍为`no_material_tail_change_detected`，继续完整epoch。进一步审计发现V48高LR point-input组只包含Ego点输入路径，World `scene_encoder/scene_context_proj/point_action_adapter`仍在低LR World组；step260的World point-action adapter漂移仅为V47的`5.52%`，而Ego adapter为`106.83%`。因此已准备但未启动V49语义修正：独立worktree`/home/liusong/ProgramFiles/Huggingface/lerobot_v49_symmetric_point_adaptation`、branch`wep_vla_v0.4.3_v49_symmetric_point_adaptation`、实现commit`231155f4cddc9ea4c2b8d10cb158d082fd41eef2`、当前HEAD`ba5429db480b088b4948383e0aaec9b09abdd742`。它仅在训练期把Ego/World两条直接点输入路径对称放入`5e-8`组，所有action/language/shared/residual路径仍以非零`5e-9`联合训练；forward、模型结构、loss与旧配置默认行为不变，并显式拒绝在legacy/不支持的fusion下静默启用。显式使用该worktree `PYTHONPATH`的CPU完整WorldFlow/SE3测试为`77/77`；真实V32 source的CPU optimizer预检确认1243个trainable tensor无重叠/遗漏：V49四组tensor数为`153/1063/25/2`，实际LR为`5e-9/5e-8/5e-9/5e-9`，两套adapter及两条scene输入投影均在高LR组；artifact为`joint_multiview_worldflow/libero10_500ep/artifacts/v49_real_checkpoint_optimizer_preflight.json`。一次未设置worktree `PYTHONPATH`的本地pytest会误加载主仓库并出现3个预期字段缺失失败，现已确认不是V49代码失败；训练入口新增硬断言，要求实际导入的`configuration_smolvla.py`必须位于V49 worktree的`src`下，否则在任何update前退出。V49当前输出目录和训练tmux均不存在，确认未训练且不会自动启动。
