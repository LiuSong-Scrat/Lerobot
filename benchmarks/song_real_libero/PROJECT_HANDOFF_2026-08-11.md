...
- v20 residual boosting gradient isolation 已完成 720 步、138,240 个样本，即完整 `10 tasks x 50 episodes` 数据的一整个 epoch。保持 v19 的前向、推理、数据、p=0.5 stochastic depth、三个非零 LR 组和原 action loss 不变；保留 World 修正的样本以当前 Ego 预测作为 stop-gradient boosting anchor，drop 样本继续正常训练 Ego。它不增加参数、门控、辅助损失或推理分支。代码 commit 为 `fe99a71`，完整测试 `67/67` 通过；W&B 正常 finished，720 个 keep-ratio 的均值为 `0.500752`。最终 runtime audit 证明三组 optimizer 仍为非零 LR、78 个 World BN buffer 与 v14 位级一致、4326 个 residual 值全部非零。固定分层筛选已接管四卡并从 step100 开始，tmux 为 `wep_v043_v20_stratified_worldflow_causal_after_train`；full-500 联合门槛 waiter 为 `wep_v043_v20_full500_matched_causal_after_screen`。
