...
- v26 是当前 active 实验。代码 commit `90afd26` 新增默认关闭的训练期 World 坐标系 SE(3) 重参数化：对 World 点云、carrier `C` 和整条 World 轨迹一致施加随机 `A`，使 `G'=(AC)B(AC)^-1=A G A^-1`，映回 Ego 后仍严格得到同一物理动作 `B`。它直接替换原单次 action-loss forward 的 World 表示，不增加第二次 forward、辅助损失、参数、门控或推理操作；eval 模式完全不采样。完整 WorldFlow/SE(3) GPU 测试 `69/69` 通过。v26 以表现最接近目标的 v21 p=0.75 配置为受控基座，仅新增坐标系增强（translation `0.05m`、rotation `0.2rad`），World 四项辅助 loss 仍全为 0，Ego/World/residual 继续共同训练。
- v26 已在 tmux `wep_v043_libero10_v26_coordframe_aug_p75_720` 启动：4 GPU、batch48/GPU、effective192、worker12、完整 `10 tasks x 50 episodes` 一 epoch、steps `100/240/480/720`、W&B enabled。W&B：`https://wandb.ai/liusong-scrat/wep_vla_v043_libero10/runs/worldflow-v26-v14zero-worldbnfixed-p75-anchor-coordframe-t005-r020-20260814`。step100 runtime audit 已通过：三个 optimizer group 全部非空且 LR 非零，78 个 World BN buffer 与 v14 逐位一致，4,326 个 residual 参数全部非零，四项 World auxiliary loss 都为0，日志中 conjugacy error 接近0且未出现非有限值；artifact 为 `v26_step000100_coordframe_aug_p75_runtime_audit.json`。Broad/Formal waiters 为 `wep_v043_v26_stratified_worldflow_causal_after_train` 与 `wep_v043_v26_full500_matched_causal_after_screen`；只有 Broad 同时严格超过 `95/100` 且 same-checkpoint World 因果增益为正才进入 Full-500，最终必须超过 established/matched baseline 并保持 Full-500 World 因果增益为正。
...
- 当前 WorldFlow 代码 HEAD：`90afd26`；World residual gradient isolation 的代码 commit 为 `fe99a71`。
