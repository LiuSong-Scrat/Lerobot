...
- V47验证链已经预注册并在tmux等待：`wep_v043_v47_convergence_monitor`在`240/480/720/960/1200/1440`分别生成loss窗口与尾部趋势审计，用来防止把未收敛误判为结构失败；同一监控还会在step240和step1440逐tensor对比v32源权重，严格检查四个优化器组以及Ego/World两套point-action adapter、共享action expert、Ego action I/O和双向cross-attention均发生非零更新，并校验checkpoint optimizer state的四组名称/顺序、非空参数、统一初始有效LR `5e-8`和正的当前LR，脚本为`audit_v47_parameter_drift.py`。该审计只读checkpoint，不改变模型、loss或训练；最终筛查被硬性约束为等待收敛和参数审计均通过后才能启动。`wep_v043_v47_step1440_stratified_3arm`依次运行双视角+World、同checkpoint主视角+World、双视角+World消融，只有三项同时满足`dual>95`、`dual>primary`、`dual>world-ablated`才进入Full500。`wep_v043_v47_step1440_full500_2x2_gate`随后运行完整2x2；最终联合臂必须`>472` baseline、`>=475` v32 WorldFlow、不低于既有WorldFlow且同时严格超过同checkpoint primary和world-ablated。正常臂第26个失败即数学早停，所有partial/cache保留。脚本与日志都统一位于experiment root，不在其他目录散放。
