...
- v46 是当前新的通用输入 coreset，独立worktree为`/home/liusong/ProgramFiles/Huggingface/lerobot_v40_v32_novelty_union`，分支`wep_vla_v0.4.3_v46_multiscale_novelty_union`，commit/HEAD `9b63311bd23392d21cf164745ba5437aabc89ad6`。方法在1cm细网格保护全部主视角覆盖，在3cm粗网格只接纳仍然缺失的副视角cell且每cell一个代表；插入数量完全由几何覆盖决定，不分配视角quota。真实数据30样本审计为严格10,000点，副视角插入均值`89.3`、范围`1--616`，并验证主视角细cell、主视角gripper尾部和副视角粗cell约束。完整测试`109 passed`。zero-shot固定分层为`93/98`后第5个失败，最高95；在已完成状态上稳定恢复V32的4个失败但新增5个失败，因此zero-shot被拒绝，不过`-1`且稳定恢复失败状态，保留一次充分paired训练的潜力。
- v46全量cache正在4卡tmux `wep_v043_v46_multiscale_novelty_cache_4gpu`生成，输出为`joint_multiview_worldflow/libero10_500ep/pointseg_cache_multiscale_novelty_union_1cm3cm`。cache按4个rank、每rank9个4096-sample shard写入；网络盘每个约1.5--1.7GB shard落盘较慢，但进程、临时文件和`/proc/io`均持续前进，不是死锁。smoke cache的4个分布式shard已逐值验证落盘`point_indices`与在线V46完全一致、严格10k、索引唯一且gripper尾部一致。全量manifest完成后，tmux `wep_v043_v46_cache_exact_index_audit`会对36个shard各取中点做同样的exact-index审计；V47训练启动器已被硬性约束为必须等待该36-shard artifact通过。不得删除或清理已有partial shard。
- cache manifest完成后，tmux `wep_v043_v47_wait_cache_then_train`会自动启动`wep_v043_v47_multiscale_novelty_paired_train`。V47从权威v32 step100开始，双/单视角paired coverage覆盖`275,180`样本，4 GPU、batch48/GPU、worker12、1440步（至少一个完整paired epoch）、W&B enabled，四组参数统一有效LR `5e-8`，无冻结、无新增辅助loss或门控；输出为`joint_multiview_worldflow/libero10_500ep/training/v47_from_v32step100_multiscale_novelty1cm3cm_paired_uniformlr5e8_4gpu_b48_w12_1440steps`。
- V47验证链已经预注册并在tmux等待：`wep_v043_v47_convergence_monitor`在`240/480/720/960/1200/1440`分别生成loss窗口与尾部趋势审计，用来防止把未收敛误判为结构失败；同一监控还会在step240和step1440逐tensor对比v32源权重，严格检查四个优化器组以及Ego/World两套point-action adapter、共享action expert、Ego action I/O和双向cross-attention均发生非零更新，并校验checkpoint optimizer state的四组名称/顺序、非空参数、统一初始有效LR `5e-8`和正的当前LR，脚本为`audit_v47_parameter_drift.py`。该审计只读checkpoint，不改变模型、loss或训练；最终筛查被硬性约束为等待完整一轮训练、收敛和参数审计均通过后才能启动。为防止只测末步而遗漏中期峰值，tmux `wep_v043_v47_all_checkpoints_stratified_3arm`会统一筛查`240/480/720/960/1200/1440`六个checkpoint；每个先运行双视角+World，达到第5个失败便数学早停，只有`dual>95`才继续同checkpoint主视角+World及双视角+World消融，三项同时满足`dual>95`、`dual>primary`、`dual>world-ablated`才成为候选。候选按正常分数、最弱因果增益及步数排序，tmux `wep_v043_v47_all_candidates_full500_2x2_gate`依次运行完整2x2并在首个通过者处停止；最终联合臂必须`>472` baseline、`>=475` v32 WorldFlow，同时严格超过同checkpoint primary和world-ablated。Full500正常臂第26个失败即数学早停，所有partial/cache保留。脚本与日志都统一位于experiment root，不在其他目录散放。
