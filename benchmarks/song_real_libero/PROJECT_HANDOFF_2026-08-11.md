...
- v46全量cache正在4卡tmux `wep_v043_v46_multiscale_novelty_cache_4gpu`生成，输出为`joint_multiview_worldflow/libero10_500ep/pointseg_cache_multiscale_novelty_union_1cm3cm`。cache按4个rank、每rank9个4096-sample shard写入；网络盘每个约1.5--1.7GB shard落盘较慢，但进程、临时文件和`/proc/io`均持续前进，不是死锁。不得删除或清理已有partial shard。
