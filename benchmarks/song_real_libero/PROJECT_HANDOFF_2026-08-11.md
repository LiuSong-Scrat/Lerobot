...
- 当前正式续跑为V47s：输出`joint_multiview_worldflow/libero10_500ep/training/v47s_from_v32step100_multiscale_novelty1cm3cm_paired_uniformlr5e8_devicebound_4gpu_b44_w12_1564steps`，tmux仍为`wep_v043_v47_multiscale_novelty_paired_train`。它使用4 GPU、batch44/GPU、worker12、1564步和W&B；全局batch176，`1564*176=275264`，覆盖完整275180 paired样本并只超出84样本。四组参数统一初始有效LR为`5e-8`，无冻结、无新增辅助loss或门控。W&B run为`joint-v47s-v32step100-multiscale-novelty-paired-uniformlr5e8-devicebound-b44-20260815`。截至step260训练无OOM/NaN，已越过两个旧故障点；四卡各只有一个训练rank。step260参数审计通过：四个optimizer组全部非零更新且组名/顺序/非空参数正确，当前LR均为`4.7000619e-8`；Ego/World两套point-action adapter、共享action expert、Ego action I/O和双向cross-attention全部发生非零漂移。artifact为`joint_multiview_worldflow/libero10_500ep/artifacts/v47_parameter_drift_source_v32step100_to_step000260.json`。
- V47s验证链已经预注册并在tmux等待：`wep_v043_v47_convergence_monitor`在`260/520/780/1040/1300/1564`分别生成loss窗口与尾部趋势审计；step260长窗口的action loss后半相对前半`+0.87%`、World flow loss`+1.43%`，当前状态为“尚未实质下降且Ego action有轻微恶化”，仅作未收敛/负迁移预警，不能替代rollout gate。该审计只读checkpoint，不改变模型、loss或训练；最终筛查被硬性约束为等待完整一轮训练和最终收敛/参数审计完成后才能启动。tmux `wep_v043_v47_all_checkpoints_stratified_3arm`会统一筛查上述六个checkpoint；每个先运行双视角+World，达到第5个失败便数学早停，只有`dual>95`才继续同checkpoint主视角+World及双视角+World消融，三项同时满足`dual>95`、`dual>primary`、`dual>world-ablated`才成为候选。候选按正常分数、最弱因果增益及步数排序，tmux `wep_v043_v47_all_candidates_full500_2x2_gate`依次运行完整2x2并在首个通过者处停止；最终联合臂必须`>472` baseline、`>=475` v32 WorldFlow，同时严格超过同checkpoint primary和world-ablated。Full500正常臂第26个失败即数学早停，所有partial/cache保留。脚本与日志都统一位于experiment root，不在其他目录分散存放。
