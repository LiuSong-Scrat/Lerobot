...
- 下一控制实验 v20 使用 residual boosting gradient isolation：保持 v19 的前向、推理、数据、p=0.5 stochastic depth、三个非零 LR 组和原 action loss不变；对保留 World 修正的样本，将当前 Ego 预测作为 stop-gradient boosting anchor，使主 action loss 更新 World/残差去学习剩余误差；丢弃修正的样本继续正常更新 Ego。它不增加参数、门控、辅助损失或推理分支，Ego/World 在同一 batch 中共同训练。默认新开关为 false，旧 checkpoint 完全兼容；完整测试 `67/67` 通过。
