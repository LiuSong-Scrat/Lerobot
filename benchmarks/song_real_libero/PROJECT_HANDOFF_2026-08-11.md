...
- v20 residual boosting gradient isolation 已从 v14 exact-zero-residual 起点启动：保持 v19 的前向、推理、数据、p=0.5 stochastic depth、三个非零 LR 组和原 action loss 不变；对保留 World 修正的样本，将当前 Ego 预测作为 stop-gradient boosting anchor，使主 action loss 更新 World/残差去学习剩余误差；丢弃修正的样本继续正常更新 Ego。它不增加参数、门控、辅助损失或推理分支，Ego/World 在同一 batch 中共同训练。代码 commit 为 `fe99a71`，完整测试 `67/67` 通过；4-GPU 训练 tmux 为 `wep_v043_libero10_v20_residual_anchor_stopgrad_720`，W&B run 为 `worldflow-v20-v14zero-worldbnfixed-w2e-p50-residual-anchor-stopgrad-20260814`。固定分层筛选与 full-500 联合门槛 waiter 也已启动，分别为 `wep_v043_v20_stratified_worldflow_causal_after_train` 和 `wep_v043_v20_full500_matched_causal_after_screen`。
