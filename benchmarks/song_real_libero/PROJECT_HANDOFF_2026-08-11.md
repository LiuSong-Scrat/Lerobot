...
- v34 已完整结束 `720/720`，checkpoint `100/240/480/720` 均落盘且训练日志无 OOM/NaN。新的固定 action baseline Full-500 已在 tmux `wep_v043_baseline_v042_fixed_action_full500_after_v34_train` 中运行：本地4卡、fixed-barrier30、推理batch30、seed0、canonical exact-action cache `read_write`。历史 `481/500` 仍是 established reference，但不能冒充这个新协议的结果；只有 artifact `taskbalanced_baseline_v042_fixed_action_step030000_4gpu_total30_b30_alltasks50ep_codefbfacd7_fixedbarrierv18_canonical_fixed_action_v34protocol.json` 完成后才能报告 matched baseline。v34 Broad/Full waiter会在该 artifact 后自动继续。
